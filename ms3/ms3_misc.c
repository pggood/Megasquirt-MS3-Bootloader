...
void antilag()
{
    ...
    if (ram_window.pg24.als_in_pin & 0x1f) {
        ...
        else if (outpc.clt > ram_window.pg24.als_minclt
                 && outpc.clt < ram_window.pg24.als_maxclt
                 && (outpc.rpm > ram_window.pg24.als_minrpm)
                 && (outpc.rpm < ram_window.pg24.als_maxrpm)
...
void traction()
{
    ...
    slipxtime = 0;
    if (((ram5.tc_opt & 1) == 0) || !GPIO_ACTIVE(tcenin)) {
...
void pitlim(void)
{
    ...
    if ((ram_window.pg28.pitlim_opt & PITLIM_OPT_ON) == 0 || GPIO_ACTIVE(plenin)) {
        // disabled or enable input on but inactive
        flagbyte24 &= ~FLAGBYTE24_PITLIMON;
        skipinj_pitlim = 0;
    }
    else if (GPIO_ACTIVE(plenin)) {
